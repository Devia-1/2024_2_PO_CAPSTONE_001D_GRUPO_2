...
function verificarTablas() {
    ...
    $sentencia  = $bd->query("SELECT COUNT(*) AS resultado FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'cafealasproyecto'");
...
function iniciarSesion($correo, $password) {
    $bd = conectarBaseDatos();
    // Realizamos un JOIN entre usuarios y cargo para obtener id_cargos y descripcion
    $sentencia = $bd->prepare("
        SELECT u.id, u.nombre, u.id_cargos, c.descripcion, u.password
        FROM usuarios u
        LEFT JOIN cargo c ON u.id_cargos = c.id_c
        WHERE u.correo = ?
    ");
    ...
    elseif ($sentencia->rowCount() == 1) {
        $passwordVerifica = password_verify($password, $usuario->password);
        if ($passwordVerifica) {
            // Ahora $usuario tendrá id_cargos y descripcion del cargo
            return $usuario;
        } else {
            return false;
        }
